...
local function get_info_registry(host)
  ...
  -- TODO: Known DLLs?
  result['status-bios'], result['bios'] = reg_get_value(smbstate, openhklm_result['handle'], "HARDWARE\\DESCRIPTION\\System\\BIOS", "SystemManufacturer")
...
action = function(host)
  ...
  if(result['status-os'] == true) then
    ...
    table.insert(hardware, string.format("BIOS: %s", result['bios']))
